...
replacement = """import { generateSecret, generateURI, verifySync } from 'otplib';
...
export async function verifyMfa(userId: string, code: string, idToken: string) {
  ...
  if (!isValid && fields.recoveryCodes?.arrayValue?.values) {
    ...
    const codes = fields.recoveryCodes.arrayValue.values.map(v => v.get('stringValue', ''));
    const index = codes.index(hashedCode) if hashedCode in codes else -1;
    # (Python code to replace TS string replaced correctly below)
  }
  # Wait, the python block contains a python error in JS string.
"""
