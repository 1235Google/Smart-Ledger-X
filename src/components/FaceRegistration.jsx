/**
 * FaceRegistration.jsx - Optimized for Accuracy & Robust Enrollment
 * 
 * KEY ACCURACY & PERFORMANCE OPTIMIZATIONS:
 * -------------------------------------------------------------
 * 1. HIGH-ACCURACY DETECTOR (SSD MobileNet v1):
 *    - Uses faceapi.SsdMobilenetv1Options() instead of TinyFaceDetector for registration,
...
import { Camera, CheckCircle2, AlertTriangle, RefreshCw, X, Sparkles, Zap } from 'lucide-react';
...
export default function FaceRegistration({ onComplete, onCancel, isOpen = true }) {
  ...
  const [statusMessage, setStatusMessage] = useState("Position your face in the circular frame");
   ...
   */
  const handleSampleCaptured = (descriptor, score) => {
   ...
   */
  const finishRegistration = async (descriptors) => {
      ...
      >
        ...
        {loadError ? (
          ...
          <div className="relative mb-5">
            ...
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-neutral-900/90 border border-white/20 text-white text-[11px] font-medium shadow-lg flex items-center gap-1.5 whitespace-nowrap">
