      ...
      <AnimatePresence mode="wait">
        {!selectedDate ? (
          ...
          >
            ...
            <div className="relative bg-gradient-to-br from-[#0B1026] via-[#1a1b4b] to-[#2D4DFF] p-5 sm:p-8 rounded-2xl sm:rounded-[2rem] border border-white/10 overflow-hidden shadow-[0_20px_40px_-15px_rgba(45,77,255,0.4)] backdrop-blur-xl">
              ...
              <motion.div 
                ...
                className="absolute top-8 right-8 w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-gradient-to-tr from-white/10 to-white/30 backdrop-blur-2xl border border-white/20 shadow-[0_8px_32px_rgba(255,255,255,0.1),inset_0_4px_16px_rgba(255,255,255,0.2)] flex items-center justify-center opacity-60 sm:opacity-80 pointer-events-none"
              >
                <svg className="w-12 h-12 sm:w-16 sm:h-16 text-white/30" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1.41 16.09V20h-2.67v-1.93c-1.71-.36-3.16-1.46-3.27-3.4h1.96c.1 1.05.82 1.87 2.65 1.87 1.96 0 2.4-.98 2.4-1.59 0-.83-.44-1.61-2.67-2.14-2.48-.6-4.18-1.62-4.18-3.97 0-1.72 1.39-2.84 3.11-3.21V4h2.67v1.95c1.86.45 2.79 1.86 2.85 3.39H14.3c-.05-1.11-.64-1.87-2.22-1.87-1.5 0-2.4.61-2.4 1.61 0 .93.61 1.47 2.69 1.95 2.62.61 4.16 1.71 4.16 4.11 0 1.91-1.35 3.09-3.13 3.48z"/></svg>
              </motion.div>
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                  ...
                  <motion.div 
                    ...
                    className="text-3xl sm:text-5xl md:text-6xl font-[800] tracking-tight bg-gradient-to-b from-white to-white/70 bg-clip-text text-transparent mb-4 sm:mb-6"
                  >
                    {formatCurrency(historicalBalance)}
                  </motion.div>
                  <div className="flex gap-4">
                    <div className="bg-white/[0.03] backdrop-blur-md px-3.5 sm:px-4 py-2 rounded-xl border border-white/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
                      <p className="text-[10px] text-white/50 uppercase tracking-widest font-semibold">Starting Balance</p>
                      <p className="text-base sm:text-lg font-bold text-white/90">{formatCurrency(startingBalance)}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="bg-white/5 border border-white/10 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-6 backdrop-blur-xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-6 opacity-20 group-hover:opacity-40 transition-opacity">
                  <ArrowDownLeft size={48} className="text-green-500" />
                </div>
                <div className="text-slate-400 text-xs sm:text-sm font-medium mb-1 uppercase tracking-wider">Total Received (Snapshot)</div>
                <div className="text-2xl sm:text-3xl font-bold text-white mb-4">{formatCurrency(totalReceived)}</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-6 backdrop-blur-xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-6 opacity-20 group-hover:opacity-40 transition-opacity">
                  <Clock size={48} className="text-amber-500" />
                </div>
                <div className="text-slate-400 text-xs sm:text-sm font-medium mb-1 uppercase tracking-wider">Total Pending (Snapshot)</div>
                <div className="text-2xl sm:text-3xl font-bold text-white mb-4">{formatCurrency(totalPending)}</div>
            ...
            {chartData.length > 0 && (
              <div className="bg-white/5 border border-white/10 rounded-2xl sm:rounded-[2.5rem] p-4 sm:p-6 backdrop-blur-xl">
                <h3 className="text-xs sm:text-sm font-medium text-slate-400 mb-4 sm:mb-6 uppercase tracking-wider">Balance Trend (Leading to {formatDate(selectedDate)})</h3>
            ...
            <div>
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                <h2 className="text-lg sm:text-xl font-bold text-white">Historical Transactions</h2>
                <div className="text-xs sm:text-sm text-slate-400">{filteredTransactions.length} records</div>
              </div>
              <div className="space-y-3">
                {filteredTransactions.map((tx, idx) => (
                  <motion.div
                    ...
                    className="group vision-glass-subtle p-3.5 sm:p-4 rounded-xl sm:rounded-2xl flex items-center gap-3 sm:gap-4"
                  >
                    <div className={cn(
