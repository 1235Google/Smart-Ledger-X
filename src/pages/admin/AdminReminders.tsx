...
export default function AdminReminders() {
  ...
  return (
    <DataStateGuard
      ...
      loadingMessage="Loading scheduled reminders..."
      skeletonType="table"
    >
      <div className="space-y-8 max-w-7xl mx-auto pb-16">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button 
            onClick={() => showSuccess("Reminders Sent", "Bulk reminder dispatch triggered across all active channels.")}
            className="px-6 py-3.5 bg-gradient-to-r from-blue-600 to-emerald-500 text-white font-bold rounded-xl shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:opacity-95 transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
              ...
              pendingTransactions.map((tx, idx) => (
                <div key={tx.id || idx} className="flex flex-col md:flex-row md:items-center justify-between p-5 rounded-2xl bg-black/40 border border-white/5 gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button 
                      onClick={() => showSuccess("Reminder Dispatched", `Notification sent to ${tx.personName}!`)}
                      className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-xl text-xs flex items-center gap-2 transition-colors shadow-lg cursor-pointer"
                    >
